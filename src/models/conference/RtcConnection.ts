...
export class RtcConnection{
  ...
  public uploadFile(file:File):Promise<string>{
    ...
    const promise = new Promise<string>(async (resolve, reject)=>{
      ...
      const onCloseEvent = () => {
        ...
        console.log('onClose() rtcSocket for mainServer')
        this.disconnect()
      }
      const onErrorEvent = () => {
        console.error(`Error in rtcSocket for ${config.mainServer}`)
  ...
  private sendWithPromise(msg:MSMessage, resolve:(a:any)=>void, reject?:(reson:any)=>void, arg?:any){
    if (!this.mainServer) {
      if (reject){
        console.log("send message rejected. type:" + msg.type + ': no mainServer')
  ...
  public createTransport(dir: MSTransportDirection, remote?: RemotePeer){
    ...
    const promise = new Promise<mediasoup.types.Transport>((resolve, reject) => {
      ...
      // FIXED:ver1.0.3 remote→remote.peer
      this.sendWithPromise(msg, resolve, reject, remote?.peer)
    })
    return promise
  }
