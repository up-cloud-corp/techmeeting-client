...
export class DataSync{
  ...
  private onChatHistory(rawHistory: ServerChatMessage[]){
    const assembledHistory = rawHistory.map(raw => {
      const type:ChatMessageType = raw.to ? 'private' : 'text';
      return new ChatMessage(raw.text, raw.from, raw.name, raw.avatarUrl, raw.colors, raw.ts, type);
    });
    // Update the chat store with the retrieved history
    chat.setChatHistory(assembledHistory);
  }

  private async onCallRemote(from: string | undefined) {
    assert(from);
    ...
    if (caller) {
      ...
      // Prevent duplicate playback ver1.1
      if (participants.local.callSoundStatus === 0) {
        // Set the call sound playback status to 1 (waiting to play) to prevent duplicate playback ver1.1
        participants.local.callSoundStatus = 1;
        // playback notification sound ver1.1
        await this.callSound();
      }
    }
  }
  // playback notification sound ver1.1
  private async callSound() {
    if (participants.local.callSoundStatus !== 1) {
      return null
    }
    participants.local.callSoundStatus = 2;
    await this.playNotificationSound();
    participants.local.callSoundStatus = 0;
  }
  private playNotificationSound(): Promise<void> {
    return new Promise((resolve, reject) => {
      const audio = new Audio('/audio/notificationSounds/3tri16.35.wav');
      audio.play().then(() => {
  ...
  onBmMessage(msg: BMMessage){
    ...
    switch(msg.t){
      ...
      case MessageType.REQUEST_CHAT_HISTORY: this.onChatHistory(JSON.parse(msg.v)); break
