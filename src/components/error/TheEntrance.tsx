...
export const TheEntrance: React.FC = () => {
  ...
  const onClose = (enter: boolean) => {
    if (!enter || name.length === 0 || isLoading) return
    setIsLoading(true);
    ...
    >
      <DialogContent style={dialogStyle}>
        ...
        <p>
          ...
          />
          {t("enAbout")}&nbsp;
          <a href={t("enTopPageUrl")} target="_blank">{t("enMoreInfo")}</a>
        </p>
        <p style={{ fontSize: "0.6em"}}>
          powered&ensp;by&ensp;
          <a style={{ fontSize: "1.2em"}} href={t("enBMPageUrl")} target="_blank">{t("enBMMoreInfo")}</a>
        </p>
        <br />
        <TextField
          label={t("YourName")}
