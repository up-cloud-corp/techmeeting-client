  ...
  return <div ref={containerRef} className={classes.root}>
    <Collapse in={showFooter} classes={classes}>
      ...
      <FabMain size={fabSize} color={mute.muteV ? 'primary' : 'secondary'} aria-label="camera"
        onClick = { () => {
          // camera button lock
          let diff = Date.now() - participants.local.isTurningOffCameraTime
          if(diff < 1500 || participants.local.isTurningOffCamera) {
            return
          }
          participants.local.isTurningOffCamera = true
          participants.local.isTurningOffCameraTime = Date.now()
