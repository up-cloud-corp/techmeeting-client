...
import groupIcon from '@iconify/icons-material-symbols/group-rounded'
...
export const ShareMenu: React.FC<ShareMenuProps> = (props) => {
  ...
  const createWhiteboard = () => {
    ...
    createContentOfIframe(
      `https://wbo.ophir.dev/boards/TechMeeting_${conference.room}_${randStr}`, map).then((c) => {
      contents.shareContent(c)
       contents.setEditing(c.id)
    })
  }
  const createSameRoom = () => {
    setStep('none')
    createContentOfSameRoom(map).then(c => {
      contents.shareContent(c)
    })
  }
  const createScreen = () => {
  ...
  return (
    <List>
      ...
      />
      {/* ver1.2 added same room */}
      <DialogIconItem
        tip = {t('shareSameRoomTip')}
        key="shareSameRoom" text={t('shareSameRoom')} icon={<Icon icon={groupIcon}/>}
        onClick={createSameRoom}
      />
      <Divider />
      <DialogIconItem
        tip = {t('shareMouseTip')}
