...
import {SignalQualityIcon} from '../map/Participant/SignalQuality'
...
export const ParticipantLine: React.FC<TextLineStyle&{participant: AnyParticipant}> = (props) => {
    ...
    <Tooltip title={`${props.participant.information.name} (${props.participant.id})`} placement="right">
      <div className={classes.outer} style={{margin:'1px 0 1px 0'}}>
        <IconButton style={{margin:0, padding:0}} onClick={onClick} onContextMenu={onContextMenu}>
          ...
          {props.participant.trackStates.speakerMuted ? <SpeakerOffIcon style={iconR} color="secondary"/> :
            props.participant.trackStates.micMuted ? <MicOffIcon style={iconR} color="secondary"/> : undefined }
        ...
        {props.participant.id === participants.localId ?
          ...
          <SignalQualityIcon size="16px" quality={props.participant.quality}
           transport={conference.remotePeers.get(props.participant.id)?.transport} />
        }
      </div>
    </Tooltip>
    {props.participant.id === participants.localId ?
