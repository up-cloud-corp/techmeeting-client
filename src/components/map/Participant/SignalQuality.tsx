...
export const SignalQualityIcon: React.FC<SignalIconProps> = (props) => {
  ...
  if(props.transport?.connectionState == 'new' || props.transport?.connectionState == 'connecting'){
    qualityIcon = <CircularProgress style={{ marginTop: '-1px',color: 'gray', fontSize: 'small', width: props.size, height: props.size }}/>
  } else if(props.transport?.connectionState == 'connected'){
    if (quality !== undefined){
      qualityIcon = <SignalCellular0BarIcon className={props.className} style={{color:'red', width: props.size}} />
      if(quality > 30) { qualityIcon =
        <SignalCellular1BarIcon className={props.className} style={{color:'orange', width: props.size}} /> }
      if(quality > 50) { qualityIcon =
        <SignalCellular2BarIcon className={props.className} style={{color:'gold', width: props.size}} /> }
      if(quality > 70) { qualityIcon =
        <SignalCellular3BarIcon className={props.className} style={{color:'yellow', width: props.size}} /> }
      if(quality > 90) { qualityIcon =
        <SignalCellular4BarIcon className={props.className} style={{color:'green', width: props.size}} /> }
    } else {
      qualityIcon =
      <SignalCellular4BarIcon className={props.className} style={{color:'gray', width: props.size}} />
    }
  }
...
export const SignalQualityButton: React.FC<SignalQualityButtonProps> = (props:SignalQualityButtonProps) => {
  ...
  return <IconButton ref={ref} onClick={()=>{ setOpen(true) }}>
    <SignalQualityIcon size="24px" quality={getStatFromTransport(props.transport)?.quality} transport={props.transport}  />
